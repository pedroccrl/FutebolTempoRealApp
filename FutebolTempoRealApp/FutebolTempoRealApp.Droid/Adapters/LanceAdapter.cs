...
namespace FutebolTempoRealApp.Droid.Adapters
{
    ...
    class LanceAdapter : BaseAdapter<ItemSection<Lance>>
    {
        ...
        public LanceAdapter(LayoutInflater layoutInflater, SectionList<Lance> lances)
        {
        ...
        public override View GetView(int position, View convertView, ViewGroup parent)
        {
            ...
            if (item.IsSection)
            {
                if (view == null || view?.Id != Resource.Layout.HeaderView) view = Inflater.Inflate(Resource.Layout.HeaderView, null);
                view.FindViewById<TextView>(Resource.Id.HeaderTxtHeader).Text = item.Titulo;
            }
            else
            {
                if (view == null || view?.Id != Resource.Layout.LanceView) view = Inflater.Inflate(Resource.Layout.LanceView, null);
